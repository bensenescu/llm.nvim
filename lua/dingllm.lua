...
function M.invoke_llm_and_stream_into_editor(opts, make_curl_args_fn, handle_data_fn)
	...
	-- 2. Shut down the previous job if it exists
	if active_job then
		active_job:shutdown()
		active_job = nil
	end
	...
	local system_prompt = opts.system_prompt
		or "You are angry harvey specter. Yell at me for not setting my configuration for my llm plugin correctly"
	local args = make_curl_args_fn(opts, prompt, system_prompt)
	local curr_event_state = nil

	-- Start a new job calling the callback
	active_job = Job:new({
		command = "curl",
		args = args,
	...
	vim.api.nvim_create_autocmd("User", {
		...
		pattern = "LLM_Escape",
		callback = function()
			-- Question: why doesn't this clear the autocmds too? I'm not sure why  we clean up in the beggining of this
			-- function and also passthis callback into the autocmd
			if active_job then
				active_job:shutdown()
